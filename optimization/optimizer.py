...
def optimize_allocation(
    ...
    """
    Decide how many kg of the harvest to send to each
    destination to maximize total risk-adjusted net return.
    """
    ...
    for option in options:
        ...
        upper_bound = (
            option.capacity_kg
            if option.capacity_kg is not None
            else harvest_quantity_kg
        )

        # Never allow a destination to receive
        # more than the total harvest.
        upper_bound = min(
            float(upper_bound),
            float(harvest_quantity_kg)
        )

        allocation_vars[option.id] = solver.NumVar(
            0,
            upper_bound,
            option.id
        )
    ...
    # --------------------------------------------------------
    # Constraint:
    # Total allocated cannot exceed harvest quantity
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Calculate net and risk-adjusted prices
    # --------------------------------------------------------
    ...
    for option in options:
        ...
        net_price = calculate_net_price_per_kg(
            option
        )
    ...
    # --------------------------------------------------------
    # Objective:
    # Maximize total risk-adjusted expected return
    # --------------------------------------------------------

    solver.Maximize(
        sum(
            allocation_vars[option.id]
            * risk_adjusted_prices[option.id]
    ...
    if status != pywraplp.Solver.OPTIMAL:
        ...
        raise RuntimeError(
            "Optimizer did not find an optimal solution."
        )
    ...
    details = [

        {
            ...
            "days_to_realize":
                option.days_to_realize,

            "net_price_per_kg":
                net_prices[option.id],
...
def get_optimal_strategy(
    ...
    """
    Single entry point for the optimization system.

    Creates MARKET, BUYER and STORE destination options,
    then optimizes allocation using risk-adjusted net return.

    For STORE, the expected future selling price is evaluated
    together with the eventual reference-market costs.
    """
...
if __name__ == "__main__":
    ...
    for detail in result["details"]:
        ...
        if detail["allocated_kg"] > 0:

            print(
                f"  "
                f"{detail['allocated_kg']} kg -> "
                f"{detail['destination_id']} "
                f"({detail['kind']}) @ "
                f"₹{detail['risk_adjusted_price_per_kg']}/kg"
            )


    print(
        "\nTotal expected return: "
        f"₹{result['total_expected_return']}"
    )